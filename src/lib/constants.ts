...
// 版式规范源自 image-asset-prompt-library 技能的"角色三视图/资产页标准版式"（lib/prompts/skills/image-asset-prompt-library）
export const CHARACTER_PROMPT_SUFFIX = '角色设定图，画面分为左右两个区域：【左侧区域】占约1/3宽度，是角色的正脸特写（如果是人类则展示完整正脸，如果是动物/生物则展示最具辨识度的正面形态），眼睛平视镜头，表情中性沉静，无情绪流露；【右侧区域】占约2/3宽度，是同一角色的三视图横向均匀排列（从左到右依次为：正面全身、侧面全身（90°纯侧面）、背面全身），三视图身高完全一致、头身比例统一，服装图案与结构在各视图中空间连续对应，视图之间留均等间距，双手自然下垂呈标准站姿。所有视图保持完全一致的身份特征、面部结构、五官比例、发型、肤色、体型、服装与材质，光照方向统一。纯白色背景，无其他元素，无文字无水印。'

// 道具图片生成的系统后缀（固定白底三视图资产图）
// 版式规范源自 image-asset-prompt-library 技能的道具资产页版式（左1/3核心部位特写 + 右2/3多角度视图）
export const PROP_PROMPT_SUFFIX = '道具设定图，画面分为左右两个区域：【左侧区域】占约1/3宽度，是道具核心部位的近距离特写；【右侧区域】占约2/3宽度，是同一道具的三视图横向均匀排列（从左到右依次为：正面、侧面、背面），三视图高度完全一致，视图之间留均等间距。所有视图保持完全一致的外形轮廓、结构逻辑、材质表面与颜色，主光方向统一。纯白色背景，主体居中完整展示，无人物、无手部、无桌面陈设、无环境背景、无其他元素，无文字无水印。'

// 场景图片生成的系统后缀（质量约束，源自 image-asset-prompt-library 技能的场景质量约束模块；不指定版式，单张自由构图）
export const LOCATION_PROMPT_SUFFIX = '真实尺度与材质受光准确，空间结构与透视无畸变，画面整洁无文字无水印。'
